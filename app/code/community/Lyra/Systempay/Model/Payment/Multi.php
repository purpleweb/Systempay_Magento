...
class Lyra_Systempay_Model_Payment_Multi extends Lyra_Systempay_Model_Payment_Abstract
{
    ...
    protected function _setExtraFields($order)
    {
        ...
        $this->_systempayRequest->set('contracts', (isset($option['contract']) && $option['contract']) ? 'CB=' . $option['contract'] : null);

        $this->_getHelper()->log('Multiple payment configuration is ' . $this->_systempayRequest->get('payment_config'));
    }
    ...
    public function assignData($data)
    {
        ...
        $option = $this->_getOption($data->getSystempayMultiOption());

        $info->setAdditionalData($option ? serialize($option) : null)
                ->setCcType($data->getSystempayMultiCcType())
                ->setCcLast4(null)
                ->setCcNumber(null)
                ->setCcCid(null)
                ->setCcExpMonth(null)
    ...
    {
        // all cards
        $allCards = Mage::getModel('systempay/source_multi_paymentCards')->getMultiCards();
