...
abstract class Lyra_Systempay_Model_Payment_Abstract extends Mage_Payment_Model_Method_Abstract
{
    ...
    public function getFormFields($order)
    {
        ...
        $this->_systempayRequest->set('contrib', 'Magento1.4-1.9_1.7.1/' . Mage::getVersion());
        ...
        if ($admin) {
            ...
            $sendEmail = $session->getSystempayCanSendNewEmail(true);
            $this->_systempayRequest->set('order_info', 'send_confirmation=' . $sendEmail);
        }
        ...
        // add cart data
        Mage::helper('systempay/util')->setCartData($order, $this->_systempayRequest);

        // set other data specific to FacilyPay Oney payment ond risk assessment module
        Mage::helper('systempay/util')->setAdditionalShippingData($order, $this->_systempayRequest, $this->_proposeOney());

        $paramsToLog = $this->_systempayRequest->getRequestFieldsArray(true);
        $this->_getHelper()->log('Payment parameters : ' . print_r($paramsToLog, true));

        return $this->_systempayRequest->getRequestFieldsArray();
    }
    ...
    public function acceptPayment(Mage_Payment_Model_Info $payment)
    {
        ...
        try {
            ...
            $uuid = $payment->getAdditionalInformation(Lyra_Systempay_Helper_Payment::TRANS_UUID);
            if (!$uuid) { // retro compatibility
                $legacyTransactionKeyRequest = new \Lyra\Systempay\Model\Api\Ws\LegacyTransactionKeyRequest();
                ...
                $getPaymentUuid = new \Lyra\Systempay\Model\Api\Ws\GetPaymentUuid();
            ...
            $queryRequest = new \Lyra\Systempay\Model\Api\Ws\QueryRequest();
            $queryRequest->setUuid($uuid);

            $getPaymentDetails = new \Lyra\Systempay\Model\Api\Ws\GetPaymentDetails($queryRequest);
            ...
            $this->_getAdminSession()->addWarning($this->_getHelper()->__('Please correct this error to use Systempay web services.'));
    ...
    public function denyPayment(Mage_Payment_Model_Info $payment)
    {
        ...
        try {
            ...
            $cancelPayment = new \Lyra\Systempay\Model\Api\Ws\CancelPayment();
            $cancelPayment->setCommonRequest(new \Lyra\Systempay\Model\Api\Ws\CommonRequest());
            ...
            if ($e->getRealCode() == 1) { // merchant does not subscribe to WS option
                $notice = $this->_getHelper()->__('You are not authorized to do this action online. Please, consider updating the payment in Systempay Back Office.');
    ...
    public function validatePayment(Mage_Payment_Model_Info $payment)
    {
        ...
        try {
            ...
            $validatePayment = new \Lyra\Systempay\Model\Api\Ws\ValidatePayment();
            $validatePayment->setCommonRequest(new \Lyra\Systempay\Model\Api\Ws\CommonRequest());
            ...
            $wrapper = new Lyra_Systempay_Model_Api_Ws_ResultWrapper($validatePaymentResponse->getValidatePaymentResult()->getCommonResponse());
            $stateObject = $this->_getPaymentHelper()->nextOrderState($wrapper, $order, true);
    ...
    public function validate()
    {
        if ($this->_proposeOney()) {
            ...
            Mage::helper('systempay/util')->checkAddressValidity($billingAddress);
            Mage::helper('systempay/util')->checkAddressValidity($shippingAddress);
    ...
    public function canUseForCurrency($baseCurrencyCode)
    {
        ...
        if ($quote && $quote->getId()) {
            ...
            $currency = Lyra_Systempay_Model_Api_Api::findCurrencyByAlphaCode($currencyCode);
            if ($currency) {
                return true;
            }
        }
        ...
        $currency = Lyra_Systempay_Model_Api_Api::findCurrencyByAlphaCode($baseCurrencyCode);
        if ($currency) {
            return true;
        }
    ...
    public function refund(Varien_Object $payment, $amount)
    {
        ...
        try {
            ...
            $getPaymentDetails = new \Lyra\Systempay\Model\Api\Ws\GetPaymentDetails();
            $getPaymentDetails->setQueryRequest($queryRequest);
            ...
            if ($transStatus === 'CAPTURED') { // transaction captured, we can do refund
                ...
                $refundPayment = new \Lyra\Systempay\Model\Api\Ws\RefundPayment();
                ...
                if ($amountInCents >= $transAmount) { // transaction cancel
                    ...
                    $updatePayment = new \Lyra\Systempay\Model\Api\Ws\UpdatePayment();
            ...
            $warn = $this->_getHelper()->__('Please correct error to refund payments through Systempay. If you want to refund order in Magento, use the &laquo;Refund Offline&raquo; button.');
    ...
    {
        $currency = Lyra_Systempay_Model_Api_Api::findCurrencyByNumCode($paymentResponse->getCurrency());
    ...
    protected function checkAndGetWsApi($storeId)
    {
        ...
        $options = parse_ini_file(Mage::getModuleDir('etc', 'Lyra_Systempay') . DS . 'ws.ini') ?: array();
        ...
        require_once Mage::getModuleDir('', 'Lyra_Systempay') . DS . 'Model' . DS . 'Api'  . DS . 'Ws'  . DS . 'WsApiClassLoader.php';
        \Lyra\Systempay\Model\Api\Ws\WsApiClassLoader::register(true);

        $wsApi = new \Lyra\Systempay\Model\Api\Ws\WsApi($options);
