...
class Lyra_Systempay_Model_Payment_Oney extends Lyra_Systempay_Model_Payment_Abstract
{
    protected $_code = 'systempay_oney';
    protected $_formBlockType = 'systempay/oney';
    ...
    protected  function _setExtraFields($order)
    {
        ...
        $this->_systempayRequest->set('payment_cards', $testMode ? 'ONEY_SANDBOX' : 'ONEY');

        // set choosen option if any
        $info = $this->getInfoInstance();
        if ($info->getAdditionalData() && ($option = @unserialize($info->getAdditionalData()))) {
            $this->_systempayRequest->set('payment_option_code', $option['code']);
        }
    }
    ...
    public function assignData($data)
    {
        ...
        $option = $this->_getOption($data->getSystempayOneyOption());
    ...
    public function isAvailable($quote = null)
    {
        ...
        if ($quote->getCustomerId() && !preg_match(Lyra_Systempay_Helper_Util::CUST_ID_REGEX, $quote->getCustomerId())) {
            // customer id doesn't match FacilyPay Oney rules

            $msg = 'Customer ID "%s" does not match Systempay specifications. The regular expression for this field is %s. FacilyPay Oney payment mean cannot be used.';
            $this->_getHelper()->log(sprintf($msg, $quote->getCustomerId(), Lyra_Systempay_Helper_Util::CUST_ID_REGEX), Zend_Log::WARN);
            return false;
        }
        ...
        if (!preg_match(Lyra_Systempay_Helper_Util::ORDER_ID_REGEX, $quote->getReservedOrderId())) {
            // order id doesn't match FacilyPay Oney rules

            $msg = 'The order ID "%s" does not match Systempay specifications. The regular expression for this field is %s. FacilyPay Oney payment mean cannot be used.';
            $this->_getHelper()->log(sprintf($msg, $quote->getReservedOrderId(), Lyra_Systempay_Helper_Util::ORDER_ID_REGEX), Zend_Log::WARN);
            return false;
        }
        ...
        foreach ($quote->getAllItems() as $item) {
            ...
            if (!preg_match(Lyra_Systempay_Helper_Util::PRODUCT_REF_REGEX, $item->getProductId())) {
                // product id doesn't match FacilyPay Oney rules

                $msg = 'Product reference "%s" does not match Systempay specifications. The regular expression for this field is %s. FacilyPay Oney payment mean cannot be used.';
                $this->_getHelper()->log(sprintf($msg, $item->getProductId(), Lyra_Systempay_Helper_Util::PRODUCT_REF_REGEX), Zend_Log::WARN);
                return false;
            }
        }

        if (!$quote->isVirtual() && $quote->getShippingAddress()->getShippingMethod()) {
            $shippingMethod = Mage::helper('systempay/util')->toSystempayCarrier($quote->getShippingAddress()->getShippingMethod());
            if (!$shippingMethod) {
