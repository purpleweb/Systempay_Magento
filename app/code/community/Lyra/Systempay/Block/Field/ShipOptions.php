...
/**
 * Custom renderer for the Systempay shipping options field.
 */
class Lyra_Systempay_Block_Field_ShipOptions extends Mage_Adminhtml_Block_System_Config_Form_Field_Array_Abstract
{
    public function __construct()
    {
        $this->addColumn(
            'title',
            array(
                'label' => Mage::helper('systempay')->__('Method title'),
                'style' => 'width: 210px;',
        ...
        $this->addColumn(
            'type',
            array(
                'label' => Mage::helper('systempay')->__('Type'),
                'style' => 'width: 130px;',
        ...
        $this->addColumn(
            'speed',
            array(
                'label' => Mage::helper('systempay')->__('Rapidity'),
                'style' => 'width: 75px;',
                'renderer' => new Lyra_Systempay_Block_Field_Column_List($options)
            )
        );
        ...
        $this->addColumn(
            'delay',
            array(
                'label' => Mage::helper('systempay')->__('Delay'),
                'style' => 'width: 90px;',
                'renderer' => new Lyra_Systempay_Block_Field_Column_List($options)
            )
        );
        ...
        if ($methods && is_array($methods) && !empty($methods)) {
            foreach ($methods as $code => $name) {
                $value[uniqid('_' . $code . '_')] = array(
                        'code' => $code,
                        'title' => $name,
                        'oney_label' => Mage::helper('systempay/util')->normalizeShipMethodName($name),
    ...
    protected function _toHtml()
    {
        $script = '<script type="text/javascript">
                ...
                document.observe("dom:loaded", function() {
                    $$(
                        "select.systempay_list_type",
                        "select.systempay_list_speed",
                        "select.systempay_list_delay"
                    ).each(function(elt) {
                    ...
                    $$("select.systempay_list_delay").each(function(elt) {
                    ...
                    $$("select.systempay_list_speed").invoke("observe", "change", function() {
                    ...
                    $$("select.systempay_list_type").invoke("observe", "change", function() {
