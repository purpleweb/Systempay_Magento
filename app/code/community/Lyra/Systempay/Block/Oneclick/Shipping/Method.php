...
class Lyra_Systempay_Block_Oneclick_Shipping_Method extends Lyra_Systempay_Block_Oneclick_Shipping_Abstract
{
    ...
    public function getContext()
    {
        $parentName = $this->getParentBlock() ? $this->getParentBlock()->getNameInLayout()
            : 'systempay_oneclick_product_shipping';

        if (strpos($parentName, 'product') !== false) {
            return Lyra_Systempay_Helper_Payment::ONECLICK_LOCATION_PRODUCT;
        } elseif (strpos($parentName, 'cart') !== false) {
            return Lyra_Systempay_Helper_Payment::ONECLICK_LOCATION_CART;
        } else {
            return null;
        }
    }
